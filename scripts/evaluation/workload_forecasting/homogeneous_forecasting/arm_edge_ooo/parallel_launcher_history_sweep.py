...
# 25 counters common across arm_edge aligned files (dacapo + spec_2017).
# dacapo_kafka is missing bus_cycles and stalled_cycles_frontend;
# create_dataset will warn and fall back to available columns for that workload.
COUNTERS = (
    "cpu_cycles branch_load_misses branch_loads branch_misses branches bus_cycles "
    "cache_misses cache_references dtlb_load_misses dtlb_loads instructions "
    "itlb_load_misses itlb_loads l1_dcache_load_misses l1_dcache_loads "
    "l1_dcache_stores l1_icache_load_misses l1_icache_loads l2d_cache "
    "l2d_cache_refill l3d_cache l3d_cache_refill mem_access "
...
def _model_dir(gran, freq, h, t):
    return os.path.join(ROOT_DIR, f"models_{gran}/{MACHINE}_cpu{CPU}/{freq}GHz/horizon_{h}/timesteps_{t}")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description=f"Experiment Manager — {MACHINE}_cpu{CPU} baseline sweep")
    ...
    if args.rescue:
        ...
        _send_email(
            f"[PerfCount] {MACHINE}_cpu{CPU} rescue complete on {os.uname().nodename}",
            f"{MACHINE}_cpu{CPU} rescue finished on {os.uname().nodename}.\nTotal jobs checked: {len(all_jobs)}\n"
        )
    elif args.condense:
        ...
        _send_email(
            f"[PerfCount] {MACHINE}_cpu{CPU} sweep complete on {os.uname().nodename}",
            f"{MACHINE}_cpu{CPU} full sweep finished on {os.uname().nodename}.\nTotal jobs: {len(all_jobs)}\n"
        )

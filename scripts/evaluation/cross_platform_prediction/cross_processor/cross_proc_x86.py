...
cross_proc_x86.py
=================
Cross-processor CPU-cycle prediction for x86 heterogeneous platforms
(P-Core cpu0 <-> E-Core cpu16 on x86 desktop).

Uses the unified feature engineering and CatBoost model from shared_features.py,
keeping the model identical to cross_freq_x86.py.

Typical usage
-------------
# P-Core -> E-Core, all common frequencies
python cross_proc_x86.py \\
    --data_dir ../../../../processed_data_100M/x86_desktop_heterogeneous \\
    --out_dir  ../../../../results/cross_platform/cross_processor/x86_desktop_100M \\
    --src_cpu 0 --tgt_cpu 16

# DaCapo benchmarks only
python cross_proc_x86.py \\
    --data_dir ../../../../processed_data_100M/x86_desktop_heterogeneous \\
    --out_dir  ../../../../results/cross_platform/cross_processor/x86_desktop_100M \\
    --src_cpu 0 --tgt_cpu 16 --suite dacapo

# SPEC benchmarks only
python cross_proc_x86.py \\
    --data_dir ../../../../processed_data_100M/x86_desktop_heterogeneous \\
    --out_dir  ../../../../results/cross_platform/cross_processor/x86_desktop_100M \\
    --src_cpu 0 --tgt_cpu 16 --suite spec

# E-Core -> P-Core, strict LOOCV
python cross_proc_x86.py \\
    --data_dir ../../../../processed_data_100M/x86_desktop_heterogeneous \\
    --out_dir  ../../../../results/cross_platform/cross_processor/x86_desktop_100M \\
    --src_cpu 16 --tgt_cpu 0 --strict_loocv
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Cross-processor prediction for x86 heterogeneous platforms."
    )
    ...
    parser.add_argument("--src_cpu", required=True, type=str,
                        help="Source CPU ID (e.g. '0' for P-Core, '16' for E-Core)")
    parser.add_argument("--tgt_cpu", required=True, type=str,
                        help="Target CPU ID (e.g. '16' for E-Core, '0' for P-Core)")
    ...
    if os.path.exists(summary_path) and not args.force:
        ...
        return
    print(f"\n{'='*60}")
    print(f"  cross_proc_x86 | data: {os.path.abspath(args.data_dir)}")

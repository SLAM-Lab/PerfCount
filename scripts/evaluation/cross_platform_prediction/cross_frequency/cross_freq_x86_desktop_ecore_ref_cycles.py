...
def extract_features_dynamic(df, suffix=""):
    ...
    hardware_events = [
        ...
        "cpu_cycles" # cpu_cycles is a feature now
    ]
...
def process_fold(test_bench, train_dfs, test_df, freq_ratio, target_key, out_dir):
    try:
        ...
        # CLIPPING REMOVED: Let the model learn the raw scale mathematically
        y_tr_ratio = train_ratios 
...
def main():
    ...
    # Updated output directory to cpu16_ecore
    ...
    if os.path.exists(args.data_dir):
        for f in glob.glob(os.path.join(args.data_dir, "aligned_*.csv")):
            try:
                ...
                # Filtering for CPU 16 instead of 0

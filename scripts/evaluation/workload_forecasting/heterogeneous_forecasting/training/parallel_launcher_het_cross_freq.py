...
def _log_path(job_args, cpu, tag):
    ...
    l_dir = os.path.join(
        ROOT_DIR,
        f"logs_{gran}/{MACHINE}_cpu{cpu}_het_cross_freq_{tag}/{suite}/{freq}GHz/het_{p}/horizon_{h}/timesteps_{t}"
    )
...
def mode_run_all(jobs, cpu, tag, cbm_model_dir, max_workers):
    print(f"--- FULL SWEEP [cpu{cpu} het_cross_freq_{tag}] ({len(jobs)} total jobs) ---")
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        executor.map(lambda j: run_job(j, cpu, tag, cbm_model_dir), jobs)


def mode_rescue(jobs, cpu, tag, cbm_model_dir, max_workers):
    missing = [j for j in jobs if not is_job_successful(_log_path(j, cpu, tag)[1])]
    print(f"--- RESCUE [cpu{cpu} het_cross_freq_{tag}]: {len(missing)} missing/failed ---")
    if missing:
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            executor.map(lambda j: run_job(j, cpu, tag, cbm_model_dir), missing)
    else:
...
def mode_condense(jobs, cpu, tag):
    for target_gran in GRANULARITIES:
        ...
        out_file = os.path.join(
            SCRIPT_DIR, f"het_cross_freq_{tag}_condensed_{target_gran}_cpu{cpu}.csv")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description=f"Cross-frequency het history sweep — {MACHINE}"
    )
    ...
    cbm_model_dir = None if args.no_translate else os.path.abspath(
        os.path.join(SCRIPT_DIR, f"../../../../../results/cross_platform/cross_freq/x86_10M/cpu{cpu}")
    )
    ...
    if cbm_model_dir:
        print(f"CBM model dir: {cbm_model_dir}")

    if args.rescue:
        mode_rescue(all_jobs, cpu, tag, cbm_model_dir, max_workers)
        _send_email(
            f"[PerfCount] cpu{cpu} het_cross_freq_{tag} rescue complete on {os.uname().nodename}",
            f"cpu{cpu} het_cross_freq_{tag} rescue finished.\nJobs checked: {len(all_jobs)}\n"
        )
    elif args.condense:
        mode_condense(all_jobs, cpu, tag)
    else:
        mode_run_all(all_jobs, cpu, tag, cbm_model_dir, max_workers)
        _send_email(
            f"[PerfCount] cpu{cpu} het_cross_freq_{tag} sweep complete on {os.uname().nodename}",
            f"cpu{cpu} het_cross_freq_{tag} full sweep finished.\nTotal jobs: {len(all_jobs)}\n"
        )

"""Unified top-4 counter forecasting sweep for the x86 desktop.

Unified across both cores (the former per-core x86_pcore/ and x86_ecore/
launchers have been removed in favour of this one): pass
--cpu {0,16} to select the P-core (cpu0) or E-core (cpu16). The top-4 counter
set is identical for both cores, so the CPU only affects which traces are swept
and where results are written. Trained models are saved for downstream reuse.

Usage:
  python parallel_launcher_top4_sweep.py --cpu 0   [--rescue | --condense]
  python parallel_launcher_top4_sweep.py --cpu 16  [--max_workers N]
"""
import os
import sys
import glob
import subprocess
import argparse
import re
import pandas as pd
from concurrent.futures import ThreadPoolExecutor

os.environ.update({
    'OMP_NUM_THREADS': '1', 'MKL_NUM_THREADS': '1', 'OPENBLAS_NUM_THREADS': '1',
    'BLIS_NUM_THREADS': '1', 'VECLIB_MAXIMUM_THREADS': '1', 'NUMEXPR_NUM_THREADS': '1',
    'TF_NUM_INTRAOP_THREADS': '1', 'TF_NUM_INTEROP_THREADS': '1'
})

MACHINE     = "x86_desktop_heterogeneous"
VARIANT     = "top4"
EPOCHS      = 100
MEM_PER_JOB_GB = 2

FREQS        = ["1.0", "2.0", "3.0", "4.0"]
HORIZONS     = [1, 5, 10]
TIMESTEPS    = [5, 10]
MODELS       = ["dt", "mlp", "lstm", "transformer"]
GRANULARITIES = ["10M"]

CORE_LABEL = {"0": "P-Core", "16": "E-Core"}
COUNTERS = "ref_cycles cpu_cycles branch_misses instructions".split()

SCRIPT_DIR             = os.path.dirname(os.path.abspath(__file__))
ROOT_DIR               = os.path.abspath(os.path.join(SCRIPT_DIR, "../../../../../results/forecasting/"))
WORKLOAD_FORECASTING_DIR = os.path.abspath(os.path.join(SCRIPT_DIR, "../../"))

run_env = os.environ.copy()
run_env["PYTHONPATH"] = WORKLOAD_FORECASTING_DIR

# Populated from CLI args in __main__.
CPU = None


def _send_email(subject, body):
    try:
        subprocess.run(
            ["mail", "-s", subject, "-r", "mebarondeau@utexas.edu", "mebarondeau@utexas.edu"],
            input=body.encode(),
            timeout=30,
        )
    except Exception as e:
        print(f"[EMAIL] Failed to send notification: {e}")


def _available_gb():
    with open('/proc/meminfo') as f:
        for line in f:
            if line.startswith('MemAvailable:'):
                return int(line.split()[1]) / (1024 ** 2)
    return float('inf')


def get_all_jobs():
    jobs = []
    for gran in GRANULARITIES:
        data_dir = os.path.abspath(
            os.path.join(SCRIPT_DIR, f"../../../../../processed_data_{gran}/{MACHINE}")
        )
        for freq in FREQS:
            pattern = os.path.join(data_dir, "**", f"aligned_*_{freq}GHz_cpu{CPU}_phase*.csv")
            workloads = sorted(set(
                re.sub(r'_phase\d+$', '', os.path.basename(f).replace('.csv', ''))
                for f in glob.glob(pattern, recursive=True)
            ))
            for m in MODELS:
                for w in workloads:
                    for h in HORIZONS:
                        for t in TIMESTEPS:
                            jobs.append((gran, freq, h, t, m, w))
    return jobs


def is_job_successful(log_file):
    if not os.path.exists(log_file):
        return False
    with open(log_file, 'r', errors='ignore') as f:
        content = f.read()
        if not content.strip():
            return False
        if "Traceback" in content or "Killed" in content or "Error" in content or "Exception" in content:
            return False
    return True


def _log_dir(gran, freq, h, t):
    return os.path.join(ROOT_DIR, f"logs_{gran}/{MACHINE}_cpu{CPU}_{VARIANT}/{freq}GHz/horizon_{h}/timesteps_{t}")


def _model_dir(gran, freq, h, t):
    return os.path.join(ROOT_DIR, f"models_{gran}/{MACHINE}_cpu{CPU}_{VARIANT}/{freq}GHz/horizon_{h}/timesteps_{t}")


def run_job(job_args):
    gran, freq, h, t, m, workload = job_args
    l_dir = _log_dir(gran, freq, h, t)
    m_dir = _model_dir(gran, freq, h, t)
    os.makedirs(l_dir, exist_ok=True)
    os.makedirs(m_dir, exist_ok=True)

    log_file = os.path.join(l_dir, f"{workload}_{m}.log")
    if is_job_successful(log_file):
        return

    model_path = os.path.join(m_dir, f"{workload}_{m}")
    cmd = [
        sys.executable, "src/forecasting.py",
        "--benchmark", workload, "--dataset", MACHINE, "--input_counters"
    ] + COUNTERS + [
        "--model", m, "--timesteps", str(t), "--forecast_horizon", str(h),
        "--epochs", str(EPOCHS), "--batch_size", "32", "--neurons", "16",
        "--optimizer", "nadam", "--loss_function", "mae", "--name", f"{workload}_{m}",
        "--save_model_path", model_path,
    ]
    print(f"[*] {workload} | {gran} | {freq}GHz | H:{h} | T:{t} | {m}")
    with open(log_file, "w") as f:
        subprocess.run(cmd, stdout=f, stderr=subprocess.STDOUT,
                       cwd=WORKLOAD_FORECASTING_DIR, env=run_env)


def mode_run_all(jobs, max_workers):
    print(f"--- FULL SWEEP [cpu{CPU} {CORE_LABEL[CPU]} {VARIANT}] ({len(jobs)} total jobs) ---")
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        executor.map(run_job, jobs)


def mode_rescue(jobs, max_workers):
    missing = [j for j in jobs if not is_job_successful(
        os.path.join(_log_dir(j[0], j[1], j[2], j[3]), f"{j[5]}_{j[4]}.log")
    )]
    print(f"--- RESCUE [cpu{CPU} {VARIANT}]: {len(missing)} missing/failed jobs ---")
    if missing:
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            executor.map(run_job, missing)
    else:
        print("All jobs complete.")


def mode_condense(jobs):
    print(f"--- CONDENSE [cpu{CPU} {VARIANT}] ---")
    for target_gran in GRANULARITIES:
        compiled = []
        for gran, freq, h, t, m, workload in [j for j in jobs if j[0] == target_gran]:
            log_file = os.path.join(_log_dir(gran, freq, h, t), f"{workload}_{m}.log")
            record = {
                "workload": workload, "model": m, "frequency": freq,
                "horizon": h, "timesteps": t,
                "status": "Success" if is_job_successful(log_file) else "Failed/Missing"
            }
            if record["status"] == "Success":
                with open(log_file, 'r', errors='ignore') as f:
                    for key, val in re.findall(
                        r"'([^']+)'\s*:\s*(?:np\.float64\()?([-+]?\d*\.?\d+(?:[eE][-+]?\d+)?)\)?",
                        f.read()
                    ):
                        record[key] = float(val)
            compiled.append(record)
        out_file = f"condensed_results_{target_gran}_{VARIANT}_cpu{CPU}.csv"
        pd.DataFrame(compiled).to_csv(out_file, index=False)
        print(f"Condensed {len(compiled)} jobs → {out_file}")


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description=f"top-4 forecasting sweep — {MACHINE}")
    parser.add_argument('--cpu', required=True, choices=['0', '16'],
                        help='CPU core: 0=P-core, 16=E-core')
    parser.add_argument('--max_workers', type=int, default=80)
    parser.add_argument('--variant', default=VARIANT,
                        help='Output-tree variant name (default: top4). e.g. top4b for the '
                             'branches-swap counter set.')
    parser.add_argument('--counters', default=" ".join(COUNTERS),
                        help='Space-separated input counters; first is the forecast target.')
    parser.add_argument('--models', default=" ".join(MODELS),
                        help='Space-separated model classes to sweep (default: all four).')
    parser.add_argument('--horizons', default=" ".join(str(h) for h in HORIZONS),
                        help='Space-separated forecast horizons (default: 1 5 10). Use "1" to '
                             'train only what the cross-config h=1 comparison needs.')
    parser.add_argument('--timesteps', default=" ".join(str(t) for t in TIMESTEPS),
                        help='Space-separated input timesteps (default: 5 10).')
    parser.add_argument('--rescue',   action='store_true')
    parser.add_argument('--condense', action='store_true')
    args = parser.parse_args()

    CPU = args.cpu
    VARIANT  = args.variant
    COUNTERS = args.counters.split()
    MODELS   = args.models.split()
    HORIZONS  = [int(h) for h in args.horizons.split()]
    TIMESTEPS = [int(t) for t in args.timesteps.split()]

    avail = _available_gb()
    mem_cap = max(1, int(avail / MEM_PER_JOB_GB))
    max_workers = min(args.max_workers, mem_cap)
    if max_workers < args.max_workers:
        print(f"[MEM] Available: {avail:.1f} GB → capping workers at {max_workers} (was {args.max_workers})")

    all_jobs = get_all_jobs()
    if args.rescue:
        mode_rescue(all_jobs, max_workers)
        _send_email(
            f"[PerfCount] {MACHINE}_cpu{CPU} {VARIANT} rescue complete on {os.uname().nodename}",
            f"{MACHINE}_cpu{CPU} {VARIANT} rescue finished on {os.uname().nodename}.\n"
            f"Total jobs checked: {len(all_jobs)}\n"
        )
    elif args.condense:
        mode_condense(all_jobs)
    else:
        mode_run_all(all_jobs, max_workers)
        _send_email(
            f"[PerfCount] {MACHINE}_cpu{CPU} {VARIANT} sweep complete on {os.uname().nodename}",
            f"{MACHINE}_cpu{CPU} {VARIANT} full sweep finished on {os.uname().nodename}.\n"
            f"Total jobs: {len(all_jobs)}\n"
        )

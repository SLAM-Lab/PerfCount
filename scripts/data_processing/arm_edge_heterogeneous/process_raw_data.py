...
def clean_event_name(raw_event):
    ...
    # Exact dictionary mapping for ARM Edge Heterogeneous PMU counters
    mapping = {
        ...
        # Re-mapping the raw kernel events back to clean generic names
        'br_retired': 'branches',
        'l1d_cache_wb': 'l1_dcache_stores'
    }
...
def parse_perf_script_output(proc_stdout, arch="arm"):
    ...
    if not df.empty:
        # Crucial fix to prevent "cannot convert float NaN to integer" crashes
        df = df.fillna(0)
...
def main():
    ...
    # Defaults updated for arm_edge_heterogeneous
    parser.add_argument("--raw_dir", default="../../../raw_data/arm_edge_heterogeneous", help="Directory with raw .out files")
    parser.add_argument("--out_dir", default="../../../processed_data/arm_edge_heterogeneous", help="Directory to save CSVs")

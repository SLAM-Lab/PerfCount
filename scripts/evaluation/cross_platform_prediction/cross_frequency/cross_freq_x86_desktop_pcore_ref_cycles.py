...
# ==========================================
# 1. FEATURE ENGINEERING (REF_CYCLES NORMALIZED)
# ==========================================
def extract_features_dynamic(df, suffix=""):
    X = pd.DataFrame()
    
    col_instr = f"instructions{suffix}"
    col_ref = f"ref_cycles{suffix}"

    if col_instr not in df.columns or col_ref not in df.columns:
        return pd.DataFrame()

    vec_instr = df[col_instr].replace(0, np.nan)
    vec_ref = df[col_ref].replace(0, np.nan)

    # Base metrics: Instructions Per Second (IPS) and Time Per Instruction
    X["instr_per_ref"] = df[col_instr] / (vec_ref.fillna(1e-9))
    X["ref_per_instr"] = df[col_ref] / (vec_instr.fillna(1e-9))

    # Strict whitelist for CPU 0 (Intel P-Core)
    # Note: ref_cycles is removed (it's the target/denominator)
    # Note: cpu_cycles is added (it's now a feature)
    hardware_events = [
        ...
        "l1_dcache_load_misses", "l1_dcache_loads", "l1_dcache_stores", 
    ...
    for base_event in hardware_events:
        ...
        if col in df.columns:
            # Event / Instructions = Event rate per instruction
            X[f"{base_event}_per_instr"] = df[col] / (vec_instr.fillna(1e-9))
            
            # Event / Time = Event Throughput (Events per second)
            X[f"{base_event}_per_ref"] = df[col] / (vec_ref.fillna(1e-9))
...
#        y_tr_ratio = np.clip(train_ratios, 0.5, 3.0) 
        ...
        # Lower bound of 0.1 allows for up to a 10x speedup safely
        # Upper bound of 2.0 safely caps regression
        y_tr_ratio = train_ratios #np.clip(train_ratios, 0.1, 2.0)
        ...
        # When predicting time (ref_cycles), scaling behaves inversely for compute bound
        # (Faster freq = less time). 
        # A purely memory bound workload copies 1:1 (freq doesn't speed up memory)
        pred_ml = pred_ratio * src_test_val
        pred_copy = src_test_val
        
        # Linear scale assumes perfect compute bound scaling: Time = Time * (SrcFreq/TgtFreq)
        # Because ratio passed is (tgt_freq/src_freq), we use 1/ratio
        pred_scale = src_test_val * (1.0 / freq_ratio)
...
def main():
    ...
    parser.add_argument("--out_dir", default="../../../../results/cross_platform/cross_frequency/x86_desktop_heterogeneous/cpu0_pcore", help="Base output directory")
    ...
    if os.path.exists(args.data_dir):
        for f in glob.glob(os.path.join(args.data_dir, "aligned_*.csv")):
            try:
                ...
                if match and match.group('cpu') == "0":
                    df = pd.read_csv(f)
                    df.columns = [c.strip() for c in df.columns]
                    datasets["x86_cpu0"][(match.group('freq'), f"{match.group('bench')}_phase{match.group('phase')}")] = df
            except Exception as e:
    ...
    for proc_name, data_map in datasets.items():
        ...
        for src in freqs:
            for tgt in freqs:
                if src == tgt: continue
                print(f"\n  --- CPU 0 | {src}GHz -> {tgt}GHz ---")
                
                # CHANGED: Target is now strictly ref_cycles
                res = run_comparison(data_map, src, tgt, 'ref_cycles', args.out_dir, args.jobs, args.strict_loocv)

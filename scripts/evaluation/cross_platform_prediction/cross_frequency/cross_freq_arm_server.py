...
def process_fold(test_bench, train_dfs, test_df, freq_ratio, target_key, out_dir):
    try:
        ...
        # --- NEW: EXTRACT FEATURE IMPORTANCE FOR GLOBAL AVERAGING ---
...
def run_comparison(data_map, src_freq, tgt_freq, target_key, out_dir, n_jobs, strict_loocv):
    ...
    # LOOCV LOGIC
    if strict_loocv:
        # Group phases by their base workload name
        groups = {}
        for b in valid_benches:
            base_name = b.split('_phase')[0]
    ...
    # --- NEW: PROCESS AND SAVE GLOBAL AVERAGE FEATURE IMPORTANCE ---
...
def main():
    ...
    parser.add_argument("--data_dir", default="../../../../processed_data/arm_server", help="Directory containing the aligned ARM CSVs")
    parser.add_argument("--out_dir", default="../../../../results/cross_platform/cross_frequency/arm_server", help="Base output directory")
    ...
    # 1. Load ARM Data dynamically
    if os.path.exists(args.data_dir):
        aligned_files = glob.glob(os.path.join(args.data_dir, "aligned_*.csv"))
        for f in aligned_files:
            try:
                fname = os.path.basename(f)
                match = re.search(r"aligned_(?P<bench>.+)_(?P<freq>[\d\.]+)GHz_phase(?P<phase>\d+)\.csv", fname)
                ...
                if match:
                    ...
                    full_bench = f"{bench}_phase{phase}"
                    
                    df = pd.read_csv(f)
                    df.columns = [c.strip() for c in df.columns]
                    datasets["arm_server"][(freq, full_bench)] = df
            except Exception as e:
